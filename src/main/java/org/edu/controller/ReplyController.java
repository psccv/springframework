 ...
 * ...예) '/board/123' : 게시물중 123번이라는 고유한 의미를 가지도록 설계하고,
...
public class ReplyController {
	 ...
	 * 
	 * ...크롬 Talend API Tester 이용 테스트. POST, JSON RAW데이터로 전송
	 ...
	 * 
	 * ...아래 URI에서 15은 게시글 번호임.
	 * ...http://localhost:8080/replies/all/15
			Content-Type: application/json
	 ...
	 * ...'/replies/게시물번호/댓글페이지번호' + GET방식.
	 * ...http://localhost:8080/replies/15/1
			Content-Type: application/json
	 * 
	 * ...★Ajax로 호출되므로 Model을 사용하지 못하므로, 전달해야하는 데이터들을 
	 *      담기 위해 Map타입의 객체를 별도로 생성해야함.
